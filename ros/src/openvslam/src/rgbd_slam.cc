...
void callback(const sensor_msgs::ImageConstPtr& image, const sensor_msgs::ImageConstPtr& depth, std::vector<double> &track_times, const std::chrono::_V2::steady_clock::time_point tp_0, openvslam::system *SLAM){
  
    const auto tp_1 = std::chrono::steady_clock::now();
    const auto timestamp = std::chrono::duration_cast<std::chrono::duration<double>>(tp_1 - tp_0).count();
    SLAM->feed_RGBD_frame(cv_bridge::toCvShare(image, "bgr8")->image, cv_bridge::toCvShare(depth, "32FC1")->image,timestamp); //feed mask in the end [optinal]
    const auto tp_2 = std::chrono::steady_clock::now();
    const auto track_time = std::chrono::duration_cast<std::chrono::duration<double>>(tp_2 - tp_1).count();
    track_times.push_back(track_time);
}
void rgbd_tracking(const std::shared_ptr<openvslam::config>& cfg, const std::string& vocab_file_path,
    ...
    //image_transport::ImageTransport it(nh);
    message_filters::Subscriber<sensor_msgs::Image> image_sub(nh, "/d435/camera/color/image_raw", 1);
    message_filters::Subscriber<sensor_msgs::Image> depth_sub(nh, "/d435/camera/aligned_depth_to_color/image_raw", 1);
    typedef message_filters::sync_policies::ApproximateTime<sensor_msgs::Image, sensor_msgs::Image> MySyncPolicy;
    message_filters::Synchronizer<MySyncPolicy> sync(MySyncPolicy(10), image_sub, depth_sub);
    sync.registerCallback(boost::bind(&callback, _1, _2, track_times, tp_0, &SLAM));
    // run the viewer in another thread
#ifdef USE_PANGOLIN_VIEWER
    std::thread thread([&]() {
        viewer.run();
