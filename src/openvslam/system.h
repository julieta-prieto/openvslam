...
{
public:
    SLAMData():VINSInitFlag(false), TrackingStatus(-1)
    {}
    cv::Mat Tcw;
    cv::Mat Rwi;
    cv::Mat gw;
...
public:
    ...
    void SaveKeyFrameTrajectoryNavState(const std::string& filename);
    void GetSLAMData(SLAMData& slamdata);
    Mat44_t feed_monocular_frame_VI(const cv::Mat& img, const std::vector<IMUData> &vimu, const double timestamp, const cv::Mat& mask = cv::Mat{});
    //-------------------------------------------------------------------------------------------
    //-------------------------------------------------------------------------------------------
    //-------------------------------------------------------------------------------------------
