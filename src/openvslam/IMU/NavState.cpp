...
#else
void NavState::IncSmallBias(Vector3d updatedBias)
#endif
{
    ...
    // delta_biasg_i = delta_biasg_i + dbgi,    delta_biasg_j = delta_biasg_j + dbgj
    // delta_biasa_i = delta_biasa_i + dbai,    delta_biasa_j = delta_biasa_j + dbaj
Vector3d predBa = _dBias_a, predBg = _dBias_g;
#ifndef NOT_UPDATE_GYRO_BIAS
...
//    if((/*_BiasGyr+*/_dBias_g).norm() > 0.01)
//        _dBias_g = predBg;
#else
    Vector3d upd_dBa = updatedBias.segment<3>(0);

    // delta bias of accelerometer
    _dBias_a += upd_dBa;
#endif

//    if((/*_BiasAcc+*/_dBias_a).norm() > 0.1)
//        _dBias_a = predBa;
