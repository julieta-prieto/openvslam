...
void EdgeNavStatePVR::linearizeOplus()
{
    ...
    // 6.
    // For Vertex_Bias_i
    Matrix<double,9,3> JBiasi;
    JBiasi.setZero();

    // 5.1
    // J_rPij_xxx_j for Vertex_Bias_i
    JBiasi.block<3,3>(0,0) = - M.getJPBiasa();     //J_rP_dbai

    // J_rVij_xxx_j for Vertex_Bias_i
    JBiasi.block<3,3>(3,0) = - M.getJVBiasa();    //dba_i

    // J_rPhiij_xxx_j for Vertex_Bias_i
    JBiasi.block<3,3>(6,0) = O3x3;    //dba_i
#endif
...
void EdgeNavStateBias::computeError()
{
    ...
    Vector3d err;  // typedef Matrix<double, D, 1> ErrorVector; ErrorVector _error; D=6
    err.setZero();
    // 6-Dim error vector order:
    // deltabiasGyr_i-deltabiasAcc_i
    // rBiasGi - rBiasAi
    err.segment<3>(0) = rBiasA;    // bias acc error
#endif
...
#else
    _jacobianOplusXi = - Matrix<double,3,3>::Identity();
    _jacobianOplusXj = Matrix<double,3,3>::Identity();
#endif
...
//    err.segment<3>(6) = (nsprior.Get_R().inverse() * nsPVRest.Get_R()).log();
    // PVR terms
    Sophus::SO3 Riprior_inv = nsprior.Get_R().inverse();
...
//    err.segment<3>(6) = (Riprior_inv * nsPVRest.Get_R()).log();
    ...
    // eB = Bias_prior - Bias_est
    // err_ba = (ba_prior+dba_prior) - (ba+dba)
    err.segment<3>(9) = (nsprior.Get_BiasAcc() + nsprior.Get_dBias_Acc()) - (nsBiasest.Get_BiasAcc() + nsBiasest.Get_dBias_Acc());
#endif
...
#else
    _jacobianOplusXi = Matrix<double,12,9>::Zero();
    _jacobianOplusXi.block<3,3>(0,0) = - nsPVRest.Get_RotMatrix();
    _jacobianOplusXi.block<3,3>(3,3) = - Matrix3d::Identity();
    _jacobianOplusXi.block<3,3>(6,6) = Sophus::SO3::JacobianRInv( _error.segment<3>(6) );
//    _jacobianOplusXi.block<3,3>(0,0) = ( nsprior.Get_R().inverse() * nsPVRest.Get_R() ).matrix();
...
//    _jacobianOplusXi.block<3,3>(6,6) = Sophus::SO3::JacobianRInv( _error.segment<3>(6) );
    ...
    _jacobianOplusXj = Matrix<double,12,3>::Zero();
    _jacobianOplusXj.block<3,3>(9,0) = - Matrix3d::Identity();
#endif
